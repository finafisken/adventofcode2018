...
fn prepare(input: &String) -> (Vec<(i32, i32)>, (i32, i32), (i32, i32)) {
    ...
    let (min_x, min_y) = coordinates.iter().fold((i32::max_value(), i32::max_value()), |(min_x, min_y), (x, y)| (cmp::min(min_x, *x), cmp::min(min_y, *y)));
...
fn part2(input: &String, threshold: i32) -> i32 {
    let (coordinates, (max_x, max_y), (min_x, min_y)) = prepare(input);
